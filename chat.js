        ...
        try {
            ...
            if (res.ok) {
              ...
              if (Array.isArray(data)) {
                ...
                if (vnModel) {
                  ...
                  return `\n\n=== [QUAN TRỌNG] DỮ LIỆU THỊ TRƯỜNG CHUNG: VN-INDEX ===\nĐiểm số hiện tại: ${price} điểm\nBiến động: ${change >= 0 ? '+' : ''}${change.toFixed(2)} điểm\n>>> LỆNH TỐI CAO: BẮT BUỘC dùng số liệu này khi nói về điểm số VN-Index! Bạn KHÔNG được lấy số liệu từ trí nhớ hay Google Search!\n================================================================`;
                }
              }
            }
            return '';
          ...
          if (res.ok) {
            ...
            if (data && data.ticker === rawTicker && data.price) {
              return `\n\n=== [QUAN TRỌNG] DỮ LIỆU BẢNG ĐIỆN TỬ: MÃ ${rawTicker} ===\nGiá hiện tại (VND): ${data.price} đồng\nBiến động giá: ${data.change} đồng\n>>> LỆNH TỐI CAO: BẮT BUỘC dùng số liệu này khi nói về giá mã ${rawTicker}! Mọi thông tin giá cổ phiếu bạn đưa ra BẮT BUỘC phải kèm theo Link nguồn đích danh này ở cuối câu trả lời: "[Xem chi tiết mã ${rawTicker} tại mục Phân Tích](https://fireant.vn/dashboard/content/symbols/${rawTicker})"\n================================================================`;
            }
          }
        } catch (e) {
          clearTimeout(timeoutId);
